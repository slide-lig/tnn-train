...
------------------------------------------------------------------------
local LigSampler = torch.class("dp.LigSampler")
LigSampler.isLigSampler = true

function LigSampler:__init(config)
   config = config or {}
   ...
   local args, batch_size, epoch_size, ppf, gc_freq = xlua.unpack(
      ...
      'Iteration ends after an epoch (LigSampler-dependent) ',
...
function LigSampler:setup(config)
...
function LigSampler:setBatchSize(batch_size)
...
function LigSampler:batchSize()
   return self._batch_size
end

function LigSampler:report()
   return {batch_size = self._batch_size}
end

--static function. Checks dataset type or gets dataset from datasource
function LigSampler.toDataset(dataset)
...
function LigSampler:collectgarbage()
...
function LigSampler:sampleEpoch(dataset)
   dataset = dp.LigSampler.toDataset(dataset)
   ...
   return function(batch)
      ...
      batch:setup{
         ...
         indices=indices.new(_.range(self._start,stop))
      }
...
function LigSampler:sampleEpochAsync(dataset)
   dataset = dp.LigSampler.toDataset(dataset)
   local nSample = dataset:nSample()
   local epochSize = self._epoch_size or nSample
   self._start = self._start or 1
