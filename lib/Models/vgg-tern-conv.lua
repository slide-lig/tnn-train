...
for i=1,#outputChannelSize do
  for j=1,repeatConvChannel do
    model:add(nn.TernaryConnectSpatialConvolution(inputChannelSize, outputChannelSize[i], kernelSize, kernelSize, step, step, pad, pad, opt.arch.stochWeightFlag)) -- 1 input image channel, 6 output channels, 5x5 convolution kernel
    ...
    elseif opt.arch.dropout_conv > 0 then
...
for i,hiddenSize in ipairs(opt.arch.neuronPerLayerCount) do
   model:add(nn.TernaryConnectLinear(inputHeight, hiddenSize, opt.arch.stochWeightFlag))
...
model:add(nn.TernaryConnectLinear(inputHeight, #(ds:classes()) , opt.arch.stochWeightFlag ))
